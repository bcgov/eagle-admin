...
export class FormTab2018Component implements OnInit, OnDestroy {
  ...
  onPublish(): void {
    this.confirmGuard(`Are you sure you want to <strong>Publish</strong> this project under the <strong>${'2018 Environmental Assessment Act' || this.project.legislation }</strong>?`)
      .takeUntil(this.ngUnsubscribe)
      .subscribe(
        (confirmation: boolean) => {
          if (confirmation) {
